...
#ifndef _16NAR_TOOLS_MEMORY_MANAGER_H
#define _16NAR_TOOLS_MEMORY_MANAGER_H
...
public:
     ...
     void set_default_domain( MemoryDomainUniquePtr domain );
     ...
     void register_domain( MemoryDomainUniquePtr domain,
          std::uint32_t policy = 0, StaticName parent = StaticName{} );
     ...
     MemoryDomain *get_domain( StaticName name );
...
private:
     ...
     using NamesMap = std::unordered_map< StaticName, MemoryDomain * >;

     PolicyVector policies_;                                ///< memory domains grouped with policy identifier.
     NamesMap names_;                                       ///< memory domain names.
     MemoryDomainUniquePtr default_domain_;                 ///< default memory domain.
     std::vector< MemoryDomainUniquePtr > root_domains_;    ///< memory domains which have no parents.
};

} // namespace _16nar::tools

#endif // #ifndef _16NAR_TOOLS_MEMORY_MANAGER_H
