...
Использование таких полей, как `render_api`, `resources_format` и `scene_format`, должно
осуществляться пользователем. Соответствующие объекты не будут созданы автоматически. Это
сделано для освобождения основной библиотеки движка от дополнительных зависимостей.

Можно получить директорию, в которой находится текущий исполняемый файл, с помощью функции
`GameConfig::get_app_dir()`. Рекомендуется использовать её для установки `app_dir`.

Можно получить директорию, которую рекомендуется использовать для установки `app_data_dir`,
с помощью функции `GameConfig::get_app_data_dir()`. Для Windows эта функция возвращает `%appdata%`,
